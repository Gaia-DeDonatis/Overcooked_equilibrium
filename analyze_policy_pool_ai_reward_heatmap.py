...
# -*- coding: utf-8 -*-
"""
Graph 4 variant: Policy-pool heatmap using AI reward score.
...
python analyze_policy_pool_ai_reward_heatmap.py --data-root submissions --embedding-csv tsnt_thinpath.csv --output-dir figures/policy_pool_heatmap_ai_reward_until_E16 --formats png pdf

PowerShell multi-line version:

python analyze_policy_pool_ai_reward_heatmap.py `
  --data-root submissions `
  --embedding-csv tsnt_thinpath.csv `
  --output-dir figures/policy_pool_heatmap_ai_reward_until_E16 `
  --formats png pdf
"""
...
# This script is specifically for AI reward score.
DEFAULT_METRIC = "ai_reward_score"
    ...
    for keys, g in df.groupby(group_cols, dropna=False, sort=False):
        ...
        if metric == "mean_dishes_per_round":
            ...
            raise ValueError(
                ...
                "Use ai_reward_score, mean_dishes_per_round, human_steps_per_dish, "
...
def plot_participant_policy_pool_map(
    ...
    ax.set_title(
        f"{participant_id} — {label_for_metric(metric)}",
    ...
    cbar.set_label(label_for_metric(metric), fontsize=COLORBAR_FONT_SIZE)
...
def make_all_plots(
    ...
    for participant_id, p_df in episode_summary_with_coords.groupby("participant_id", sort=True):
        ...
        stem = f"policy_pool_heatmap_{safe_id}_{metric}_until_E{int(p_df['episode_index'].max())}"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Create policy-pool heatmaps using AI reward score, "
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("figures/policy_pool_heatmap_ai_reward_until_E16"),
    ...
    parser.add_argument(
        ...
        help=(
            "Metric to visualize. Default: ai_reward_score. "
            "You can also pass another numeric round_summary.csv column."
        ),
    )
...
def main() -> None:
    ...
    if args.metric not in rounds.columns and args.metric not in {
    ...
    episode_csv = args.output_dir / f"episode_policy_pool_summary_{args.metric}_until_E{args.max_episode_index}.csv"
    best_worst_csv = args.output_dir / f"episode{args.best_episode_index}_and_worst_summary_{args.metric}_until_E{args.max_episode_index}.csv"
