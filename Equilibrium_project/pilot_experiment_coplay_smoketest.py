...
MAP_CONFIGS = {
    "counter": {
        "grid_dim": [5, 8],
        "map_type": "counter",
        "policy_pool_dir": "final_policy_pool_counter_coplay",
        ...
        "log_dir": "logs_counter_coplay_test",
    },
    "circle": {
        "grid_dim": [5, 5],
        "map_type": "circle",
        "policy_pool_dir": "final_policy_pool_circle_coplay",
        "checkpoint_filename": "model_5000000.zip",
        "policy_prefix": "[coplay][flexible][circle]agent0_",
        "mac_env_id": "Overcooked-MA-equilibrium-v5",
        "log_dir": "logs_circle_coplay_test",
    },
    "thinpath": {
        "grid_dim": [5, 7],
        "map_type": "thinpath",
        "policy_pool_dir": "final_policy_pool_thinpath_coplay",
        "checkpoint_filename": "model_5000000.zip",
        "policy_prefix": "[coplay][flexible][thinpath]agent0_",
        "mac_env_id": "Overcooked-MA-equilibrium-v3",
        "log_dir": "logs_thinpath_coplay_test",
    },
}

ACTIVE_MAP = MAP_CONFIGS[MAP_TO_TEST]

LOG_DIR = os.path.join(CURRENT_DIR, ACTIVE_MAP["log_dir"])
POLICY_POOL_DIR = os.path.join(PROJECT_ROOT, ACTIVE_MAP["policy_pool_dir"])
CHECKPOINT_FILENAME = ACTIVE_MAP["checkpoint_filename"]
POLICY_PREFIX = ACTIVE_MAP["policy_prefix"]
...
def ensure_pygame_ready():
    ...
    if screen is None:
        ...
        pygame.display.set_caption(f"{MAP_TO_TEST.capitalize()} policy smoke test")
    return screen
...
def make_env():
    mac_env_id = ACTIVE_MAP["mac_env_id"]
    ...
    env_params = {
        "grid_dim": ACTIVE_MAP["grid_dim"],
        "task": ["lettuce salad"],
        "rewardList": rewardList,
        "map_type": ACTIVE_MAP["map_type"],
        "n_agent": 2,
        "obs_radius": 0,
        "mode": "vector",
...
def main():
    ...
    print(f"Testing map: {MAP_TO_TEST}")
    print(f"Policy pool: {POLICY_POOL_DIR}")
    print(f"Checkpoint filename: {CHECKPOINT_FILENAME}")
    print(f"Policy prefix: {POLICY_PREFIX}")
    print(f"MA env: {ACTIVE_MAP['mac_env_id']}")
    ...
    for policy_idx, (policy_name, model_path) in enumerate(policies, start=1):
        ...
        for r in range(1, TOTAL_ROUNDS + 1):
            pygame.display.set_caption(
                f"{MAP_TO_TEST.capitalize()} test | policy {policy_idx}/{len(policies)} | round {r}/{TOTAL_ROUNDS}"
            )
